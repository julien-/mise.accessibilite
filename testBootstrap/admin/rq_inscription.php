...
if (isset($_POST["addcompte"])) {
    ...
    //VERIF si pseudo non existant
    $reqVerifLogin = mysql_query("SELECT pseudo_etu FROM " . $tb_etudiant . " WHERE pseudo_etu = '" . $_POST["pseudo_etu"] . "'");
    if (mysql_num_rows($reqVerifLogin) > 0)
        $erreur .= "Ce pseudo (" . $_POST["pseudo_etu"] . ") existe déja !<br/>";

    //VERIF si mail déjà utilisé
    ...
    //VERIF si mail valide
    if (!verifierAdresseMail($_POST["mail_etu"])) {
        $erreur .= "Email invalide  <br/>";
    }

    //VERIF format du pseudo (sans espaces)
    if (strpos($_POST["pseudo_etu"], ' ') !== false) {
        $erreur .= "Espaces interdits dans les pseudos !<br/>";
    }

    //VERIF si la clé prof est correcte
    $rq_cleprof_VALIDE = mysql_query("SELECT valeur_cle FROM " . $tb_cle . ", " . $tb_cours . " WHERE NOT " . $tb_cle . ".id_cle = " . $tb_cours . ".id_cle");
    $cleprof_VALIDE = mysql_fetch_assoc($rq_cleprof_VALIDE);
    if ($cleprof_VALIDE["valeur_cle"] != md5($_POST["cle_prof"])) {
        $erreur .= "Clé invalide <br/>";
    }
    ...
    if ($erreur == '') {
        ...
        $redirige = mysql_query('INSERT INTO ' . $tb_etudiant . ' (nom_etu, prenom_etu, pseudo_etu, mail_etu, pass_etu, admin) '
                . 'VALUES ("' . $_POST["nom_etu"] . '", "' . $_POST["prenom_etu"] . '", "' . $_POST["pseudo_etu"] . '", "' . $_POST["mail_etu"] . '", "' . md5($_POST["pass_etu"]) . '" , 1);') or die(mysql_error());
